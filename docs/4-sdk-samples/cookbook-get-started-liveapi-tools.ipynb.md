<!-- Fetched: 2026-08-13 18:51:34 UTC -->
...
Either select the latest stable model or a preview. Note that audio-preview models that produce audio are not supported in Colab, which only displays text output.



```python
MODEL_ID = "gemini-3.1-flash-live-preview"  # @param ["gemini-2.0-flash-live-001", "gemini-live-2.5-flash-preview", "gemini-3.1-flash-live-preview"] {"allow-input": true, "isTemplate": true}
```
...
You can set `modality="TEXT"` on any of the examples to get the text version of the output.
