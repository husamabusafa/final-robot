...
<!-- Fetched: 2026-08-13 16:45:19 UTC -->
...
Grounding with Google Maps
Not available**
5,000 requests per month (free, shared across Gemini 3), then $14 / 1,000 search queries
...
Input price
Free of charge
$3.50 or $0.0053/min* (audio)

Output price (including thinking tokens)
Free of charge
$21.00 or $0.0315/min* (audio)
...
$0.09 (audio)
$1.80 / 1,000,000 tokens per hour (storage price)
...
Not available
$0.36, prompts 200k
$8.10 / 1,000,000 tokens per hour (storage price)
...
Equivalent to $0.045 per 0.5K image*
...
Not available***
5,000 free search requests per month (shared across all Gemini 3.x models), then $14 per 1,000 requests for text and image-based grounding.
...
Output price
Not available
$1.50 (text and thinking)

$30.00 (images)

Equivalent to $0.022 per 0.5K image*

$0.034 per 1K image*,

$0.050 per 2K image*,

and $0.076 per 4K image*.
...
Equivalent to $0.0336 per 1K resolution image*
...
Output price
Not available
$0.75 (text and thinking)

$15.00 (images)

Equivalent to $0.0168 per 1K resolution image*
...
Not available ([shut down](/gemini-api/docs/deprecations#gemini-2.0-models))
Not available ([shut down](/gemini-api/docs/deprecations#gemini-2.0-models))
...
Audio input price
Free of charge
$6.50 ($0.00016 per second)

Video input price
Free of charge
$12.00 ($0.00079 per frame)
...
Image input price
Not available
$0.225 ($0.00006 per image)

Audio input price
Not available
$3.25 ($0.00008 per second)

Video input price
Not available
$6.00 ($0.000395 per frame)
...
[Google Search](/gemini-api/docs/google-search#pricing)
...
5,000 free search requests per month (shared across all Gemini models), then $14 per 1,000 requests.

[Google Maps](/gemini-api/docs/maps-grounding#pricing_and_rate_limits)
500 RPD
Not available for Pro.
1,500 RPD free (limit shared for Flash and Flash-Lite)
...
[Code execution](/gemini-api/docs/code-execution#billing)
Free of charge
...
Output tokens when created, and as
Input tokens when the model uses them as part of its
iterative reasoning process.

[URL context](/gemini-api/docs/url-context#limitations)
Free of charge
Charged as input tokens per model pricing.

[Computer use](/gemini-api/docs/computer-use)
Not available
Charged as regular tokens per model pricing (e.g., standard [Gemini 3.5 Flash](#gemini-3.6-flash) pricing). See the [Gemini 2.5 Computer Use Preview](#gemini-2.5-computer-use-preview-10-2025) pricing table for legacy model rates.

[File search](/gemini-api/docs/file-search#pricing)
Free of charge
Charged for [embeddings](#gemini-embedding) at $0.15 / 1M tokens.
Retrieved document tokens charged as regular tokens per model pricing.

[Custom Tools endpoint (Gemini 3.1 Pro Preview)](/gemini-api/docs/models/gemini-3.1-pro-preview)
Not available
Same as [Gemini 3.1 Pro Preview](#gemini-3.1-pro-preview) pricing
...
All model inference is charged at standard Gemini list rates,
including input, output, and intermediate input / reasoning tokens generated
during agentic loops.

Tool usage fees apply per existing pricing
...
[Managed agents in Gemini API](/gemini-api/docs/agents)
All model inference is charged at standard Gemini list rates,
including input, output, and intermediate input / reasoning tokens generated
during agentic loops. (See [pricing details](/gemini-api/docs/agents#pricing)).
